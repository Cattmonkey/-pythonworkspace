...
def calcData(cursor, startTime, endTime, width, height, camID, tarWidth, tarHeight, lines, M):
...
def getHeatmap(startTime, endTime, camNum=2, width=1280, height=960):
	...
	for camID in range(camNum):
		with conn.cursor() as cursor:
			...
			heatmap += calcData(cursor, startTime, endTime, width, height, camID, tarWidth, tarHeight, lines, M)
	...
	cv2.imwrite("static/images/heatmap/heatmap" + "_" + str(startTime) + "_" + str(endTime) + ".jpg", img)
#	cv2.waitKey(0)
#	cv2.destroyAllWindows()
	conn.close()
	return "heatmap" + "_" + str(startTime) + "_" + str(endTime) + ".jpg"



if __name__ == '__main__':

	dt_1 = datetime.strptime("2017-10-30 14:00:00", "%Y-%m-%d %H:%M:%S")
